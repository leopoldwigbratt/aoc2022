...
pub fn solve() -> Result<(String, String), Box<dyn Error>> {
  ...
  let results: Vec<LineResult> = lines.iter().filter_map(|line| parse_line(line, &regex)).collect();
  ...
  let part2 = dirs.values().filter(|v| **v > space_required).fold(usize::MAX, |min, val| std::cmp::min(*val, min));

  Ok((part1.to_string(), part2.to_string()))
}

fn get_dirs(results: Vec<LineResult>) -> HashMap<String, usize> {
  let (_, map): (Vec<String>, HashMap<String, usize>) = results.iter().fold((Vec::new(), HashMap::new()), |(mut current_path, mut map), lr| {
    match lr {
...
fn parse_line(line: &String, regex: &Regex) -> Option<LineResult> {
  ...
  if captures.name("cd").is_some() {
    return Some(LineResult::Cd(captures.get(2).unwrap().as_str().to_string()));
  }

  if captures.name("file").is_some() {
    return Some(LineResult::File(captures.get(5).unwrap().as_str().parse().unwrap()));
  }
...
mod tests {
  ...
  use super::{REGEX_PARSE_LINE, LineResult, parse_line, get_dirs, add_to_dirs};
  ...
  fn test_parse_command() {
    let regex = Regex::new(REGEX_PARSE_LINE).unwrap();
    assert_eq!(parse_line(&String::from("$ cd .."), &regex), Some(LineResult::Cd("..".to_string())));
    assert_eq!(parse_line(&String::from("123 asd.asd"), &regex), Some(LineResult::File(123)));
  }
}